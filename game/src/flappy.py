...
class Flappy:
    ...
    async def play(self):
        ...
        while True:
            ...
            dist_to_pipe = (
                self.pipes.upper[0].x - self.player.rect.centerx
            ) / self.config.window.width

            # normalize the player's y position
            player_cords = self.player.y / self.config.window.height
            ...
            gap_center_normalized = gap_center / self.config.window.height

            # normalize the player's speed
            player_speed = self.player.vel_y / 10
